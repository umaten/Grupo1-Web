...
async function cargarDashboardIntegrado() {
    try {
        ...
        if (!resSummary.ok || !resHistory.ok) {
            throw new Error("Fallo al conectar con los endpoints");
        }
...
function actualizarTarjetas(data) {
    ...
    if (totalVentaElem) totalVentaElem.textContent = `S/ ${data.totalSales.toFixed(2)}`;
    if (transaccionesElem) transaccionesElem.textContent = data.totalTransactions;
    if (ticketPromedioElem) ticketPromedioElem.textContent = `S/ ${data.averageTicket.toFixed(2)}`;
}
    ...
    if (ventas && ventas.length > 0) {
        ventas.forEach(venta => {
            ...
            tr.innerHTML = `
                ...
                <td>${productosHtml}</td> <!-- Inyectamos la lista aquí -->
                <td><strong style="color: #27ae60;">S/ ${venta.total.toFixed(2)}</strong></td>
            `;
